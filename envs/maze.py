# Maze RL environment with image observaitons
...
class MazeEnv(gym.Env):
    def __init__(self, stochastic=True):
        ...
        Stochasticity in the env arises from the 'slip_probability' which alters the action.
            The slip action will be the clock wise directional action (LEFT -> UP, UP -> RIGHT etc)
            Example with 'slip_probability=0.2' : with 0.2 probability a "RIGHT" action may result in "DOWN"
        """ 
        ...
        if stochastic:
            self.slip = True 
        self.distinct_states = 112  # number of unique states in the environment 
    ...
    def set_state(self, state: int) -> None:
        """
        Set the current state of environment. Useful for value iteration
    ...
    def step(self, action, slip=True):
        ...
        Args:
            ...
            slip (bool, optional): Stochasticity in env. Defaults to true
        ...
        if (r_next == self.goal_pos[0]) and (c_next == self.goal_pos[0]):
    ...
    def num2coin(self, n: int):
        # Eacch element of the below tuple corresponds to a status of each coin. 0 for not collected, 1 for collected.
        coinlist = [
            (0, 0, 0),
            (1, 0, 0),
            (0, 1, 0),
