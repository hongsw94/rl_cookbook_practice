...
# Copy coding from tensorflow2 reinforcement learning cookbook
...
class GridworldV2Env(gym.Env):
    ...
    def reset(self,):
        ...
        if self.exploring_starts:
            self.state = np.random.choice(range(0, 12))
        else:
            self.state = 8
        return self.state
    ...
    def step(self, action):
        ...
        next_state = self.get_next_state(current_position=current_position, action=action)
